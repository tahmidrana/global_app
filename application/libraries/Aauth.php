...
class Aauth {
	...
	/*
	 * Custome Functions Start
	 */

	// check if a group have a specific permission
    public function group_have_permission($perm, $group=NULL)
    {
        $perm = $this->simplify_str($perm);
        if(!$group) {
            $group = $this->CI->session->userdata('user_type_id');
        }
        $perm_list = $this->CI->session->userdata('group_permissions');
        $perm_list = array_map(array($this, 'simplify_str'), $perm_list);
    ...
    public function simplify_str($str) {
    ...
    {
        $this->CI->db->select('a.*');
        $this->CI->db->from('aauth_perms a');
        if(!in_array($group_id, array(1))) {
            $this->CI->db->join('aauth_perm_to_group b', 'a.`id`=b.`perm_id`');
            $this->CI->db->where('b.`group_id`', $group_id);
        }
        $res = $this->CI->db->get();
        $data = array();
	...
	public function create_menu($menu_label, $menu_url=NULL, $parent_menu=NULL) {
		$menu_data = array(
			'menu_label' => htmlspecialchars($menu_label),
			'menu_url' => htmlspecialchars($menu_url) ? htmlspecialchars($menu_url) : NULL,
			'parent_menu' => $parent_menu
		);

		if($this->CI->db->insert('tbl_menu', $menu_data)) {
			return TRUE;
		} else {
			return FALSE;
		}
	...
	public function get_menu1() {
		...
		$this->CI->db->where('a.parent_menu', 0);
		$this->CI->db->group_by('a.ser_id');
		$this->CI->db->order_by('a.menu_level');
		...
		foreach ($menu as $val) {
			$menu_res .= '<li class="';
			if(strtolower($val->menu_label) == strtolower($this->CI->session->userdata('main_menu'))) {
			...
			$menu_res .= '"><a href="'.base_url().$val->menu_url.'" class="">';
			if($val->menu_icon){
				$menu_res .= '<i class="fa fa-2x '.$val->menu_icon.'"></i>';
			}

			$menu_res .= '<span class="title">'.$val->menu_label.'</span></a>';

			if(strtolower($val->menu_label) == 'home') {
				$menu_res .= '</li>';
			}
			$this->CI->db->select('a.*');
			$this->CI->db->from('tbl_menu a');
			...
			$this->CI->db->where('a.parent_menu', $val->ser_id);
			$this->CI->db->group_by('a.ser_id');
			$this->CI->db->order_by('a.menu_level');
			$res = $this->CI->db->get();
			$sub = $res->result();
			...
			foreach ($sub as $s) {
				$menu_res .= '<li class="';
				if(strtolower($s->menu_label) == strtolower($this->CI->session->userdata('sub_menu'))) {
					$menu_res .= 'active';
				} else {
					$menu_res .= '';
				}
				$menu_res .= '"><a href="'.base_url().$s->menu_url.'"><span class="title">'.$s->menu_label.'</span></a></li>';
			}
			$menu_res .= '</ul>';
			if(strtolower($val->menu_label) != 'home') {
				$menu_res .= '</li>';
	...
	public function html_ordered_menu($array,$parent_id = 0)
	{
		...
		foreach($array as $element) {
			if($element['parent_menu']==$parent_id) {
				...
				$menu_html .=$element['menu_label'].'</a>';
